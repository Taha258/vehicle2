...
export default async function CarDetailPage({ params }) {
  ...
  const displayPrice = car.price 
    ? car.price.replace('AED', '€').replace('AED ', '€ ')
    : 'Prezzo su richiesta';

  const specs = [
    { icon: Gauge, label: 'Chilometraggio', value: car.mileage },
    { icon: Zap, label: 'Motore', value: car.engine },
    { icon: Fuel, label: 'Carburante', value: car.fuel },
    { icon: Settings, label: 'Trasmissione', value: car.transmission },
    { icon: Calendar, label: 'Anno', value: car.year },
    { icon: Palette, label: 'Colore', value: car.bodyColor },
    { icon: Hash, label: 'ID Stock', value: car.stockId },
    { icon: Shield, label: 'Cavalli', value: car.horsepower },
  ];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6">
          ...
          <div className="bg-[#0f172a] text-white px-6 py-4 rounded-xl shadow-2xl">
            <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Prezzo</p>
            <p className="text-xl md:text-2xl font-bold" style={{ fontFamily: 'Syne, sans-serif' }}>
              {displayPrice}
            </p>
          </div>
        </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-[#0f172a] rounded-xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-3 relative z-10">
                <a 
                  href={`https://wa.me/393282677366?text=${encodeURIComponent(`Buongiorno, sono interessato alla ${car.brand} ${car.name} (${displayPrice}).`)}`}
                  target="_blank"
            ...
            {/* Extra Benefits */}
            <div className="bg-white rounded-xl p-6 md:p-8 shadow-xl border border-gray-50">
              <h2 className="text-[#0f172a] text-xs font-bold uppercase tracking-[0.2em] mb-4 text-center">Vantaggi Inclusi</h2>
              <div className="space-y-3">
                {[
                  { label: 'Garanzia Globale', value: 'Global Warranty' },
                  { label: 'Finanziamento Personalizzato', value: 'Custom Financing' },
                  { label: 'Assicurazione Veicolo', value: 'Vehicle Insurance' },
                  { label: 'Assistenza Stradale', value: 'Roadside Assist' }
                ].map((item) => (
                  <div key={item.value} className="flex items-center gap-3 bg-[#f8f9fa] p-3 rounded-xl">
                    <CheckCircle2 className="w-4 h-4 text-[#FAC104]" />
                    <span className="text-[#0f172a] text-[10px] font-bold uppercase tracking-widest">{item.label}</span>
                  </div>
                ))}
              </div>
            </div>
